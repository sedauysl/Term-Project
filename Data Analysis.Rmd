...
gendergap <- readxl::read_xlsx("data/gendergap.xlsx" , skip = 3) %>% 
  mutate(`15-64` = as.numeric(`15-64`)) %>% 
  pivot_longer(cols = Total:`Not elsewhere classified`,
               names_to = "job_title",
               values_to = "wage") %>% 
  pivot_longer(cols = `15+`:`25+`,
               names_to = "age_group",
               values_to = "underemployment")

worlddata <- gendergap %>%
  select(Country:wage) %>% 
  unique() %>% 
  group_by(Time, Sex, job_title) %>% 
...
```{r}
# Bu da pek dogru degil ama kabul ettim.
gendergap %>%
  filter(Sex %in% c("Male", "Female")) %>%
  filter(job_title == "Total", Country == "United States") %>% 
  select(Country:wage) %>% 
  unique() %>% 
  t.test(wage ~ Sex, 
         data = ., 
...
```{r}
# bu dogru ve yorumlanabilir bir test degil!!!
gendergap %>%
  filter(Sex %in% c("Male", "Female")) %>%
...
gendergap %>% 
  ...
  group_by(Sex) %>%
  nest() %>%
  mutate(t_test = map(data, ~t.test(.x$wage))) %>%
  mutate(glance_t_test = t_test %>% purrr::map(glance)) %>%
  unnest(glance_t_test) %>%
