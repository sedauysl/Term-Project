...
group_by(Sex) %>%
  nest() %>%
mutate(t_test = map(data, ~t.test(.x$Total))) %>%
...
mutate(t_test = map(data, ~t.test(.x$Total))) %>%
  mutate(glance_t_test = t_test %>% purrr::map(glance)) %>%
  unnest(glance_t_test) %>%
  select(Country, statistic, p.value)
```
...
Male technicians and associate professionals Avg Earngs according to country
```{r}
gendergap %>%
  janitor::clean_names() %>%
  group_by(country) %>% 
...
female technicians and associate professionals Avg Earngs according to country
```{r}
gendergap %>%
  janitor::clean_names() %>%
  group_by(country) %>% 
