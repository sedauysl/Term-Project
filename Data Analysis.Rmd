...
gendergap2$`Employment.Rate` <- as.numeric(gendergap2$`Employment.Rate`)
...
gendergap2 %>%
  filter(Sex %in% c("Female", "Male")) %>%
   filter(Economic.Activity %in% c("Business and Administrative Services", "Agriculture", "Manufacturing", "Construction", "Business and Administrative Services", "Public Administration and other Services")) %>%
      ...
      labs(subtitle = "Wage Rate According to Economic Activity",
           x = "Wage",
           y = "Economic Activity") +
...
gendergap2 %>%
  filter(Sex %in% c("Female", "Male")) %>%
      ...
      geom_col( position = "dodge") +
      labs(subtitle = "Unemployment by Education Level",
           x = "Unemployment",
           y = "Education Level") +
      hrbrthemes::theme_ipsum_rc()
```
...
gendergap2 %>%
  filter(Sex %in% c("Female", "Male")) %>%
  na.omit() %>%
  ggplot(aes(x = Sex, y = Employment.Rate)) +
  geom_boxplot() +
  labs(x = "Sex",
       y = "Employment Rate")
```
...
janitor::clean_names() %>%
  filter(sex %in% c("Male", "Female")) %>%
  unique() %>% 
  t.test(employment_rate ~ sex, 
         data = ., 
         alternative = "two.sided", confidence.level = 95) %>% 
  tidy() %>% 
  select(estimate, statistic, p.value) %>% 
  knitr::kable()
...
janitor::clean_names() %>%
  filter(sex %in% c("Male", "Female")) %>%
  select("area", "wage", "sex", "education", "unemployment_by_education_level") %>% 
  unique() %>% 
  t.test(unemployment_by_education_level ~ sex, 
         data = ., 
         alternative = "two.sided") %>% 
  tidy() %>% 
  select(estimate, statistic, p.value) %>%
...
gendergap2 %>%
  filter(Sex != "Total") %>%
  filter(Sex != "Female") %>%
  select("Sex", "Wage", "Unemployment.by.Education.Level", "Unemployment.by.Age", "Employment.Rate") %>% 
